...
template <typename T> double benchmarkStdVectorPushBack(long long iterations) {
...
inline int vec_bench() {
  ...
  for (long long iteration : iters) {
    ...
    std::cout << "Winner: "
              << ((custom_int_time < std_int_time) ? "Custom Vector"
                                                   : "Std Vector")
              << "\n";
    std::cout << "Faster by: "
              << ((custom_int_time < std_int_time)
                      ? (std_int_time - custom_int_time)
                      : (custom_int_time - std_int_time))
              << " ms\n";
    std::cout << "---------------------------------\n";
  }
